...
cleanDataByThresholdingSeverity <- function(data, threshold) {
  return (data %>% mutate_at(.vars = c("severity"), .funs = function(s) { return (s > threshold) }))
}
...
#-- Function: mean_ordered --------------------------------------------------
mean_ordered <- function(x)
{
  4 - mean(as.numeric(x))
}
  ...
  function(data) {
    ...
    dataGrouped <- data %>%
      ...
      dataCleaned <- dataGrouped %>%
        ...
        mutate(severity = mean_ordered(severity)
	  #mean(severity)
        ) %>%
  ...
  if (smell == "blob" |
    ...
    imputeResult <- mlr::impute(
      ...
      classes = list(
        ...
        numeric = mlr::imputeLearner(mlr::makeLearner("regr.rpart")),
        #takes care of double values!
        factor = mlr::imputeLearner(mlr::makeLearner("classif.rpart"))
      ),
       dummy.classes = dummyFeatures
    )
...
measuremulticlass.AvFbetaCalc <- function(confMatrix, levels) {
...
makeStats <- function(model) {
  matrix <- mlr::calculateConfusionMatrix(model, relative = FALSE, sums = FALSE)
...
measuremulticlass.AvFbeta = function(pred){
  ...
  cm <- mlr::calculateConfusionMatrix(pred,relative=FALSE)
  confMatrix <- cm$result[1:levels,1:levels]
  return(measuremulticlass.AvFbetaCalc(confMatrix, levels))
}


multiclass.AvFbeta = mlr::makeMeasure(
...
myMakeLearnerNAI <- function(learnerName, id){
  return(
  makeImputeWrapper(
  learner = makeLearner(learnerName, id),
  classes = list(
...
rep20cv10stratify <- mlr::makeResampleDesc("RepCV", folds = 10L, reps = 20L, stratify = TRUE)


print("Utility functions sourced")
