...
targetRoot <- "../../images/2022-04-01-repro-prec"
...
calculateStatsRepro <- function(directory, threshold) {
    ...
    smellmetrics_summaries <- smellmetrics %>% 
      unite("key", c(smell, algorithm, metric, threshold), sep="/")  %>% 
      group_by(key) %>%
      summarize(
      ...
      separate(key, c("smell", "algorithm", "metric", "threshold"), sep="/")
...
for(smell in names(smellsToAnalyze)) {
    ...
    for(ds in names(datasetsToAnalyze)) {
        ...
        localstatsfull <- calculateStatsRepro(datadir, datasetsToAnalyze[ds])
        stats_full <- bind_rows(stats_full, localstatsfull[[1]])
	localstats <- localstatsfull[[2]] %>% filter(metric %in% names(METRIC_NAME_MAPPING))
        stats_table <- bind_rows(stats_table, localstats)
    }
    ...
    write.csv(stats_full[[1]], paste(targetRoot, paste(paste("full-results",smell, sep="_"), "csv", sep="."), sep="/"))
    write.csv(stats_full[[2]], paste(targetRoot, paste(paste("summaries",smell, sep="_"), "csv", sep="."), sep="/"))
    
    stats_boxplot <- stats_full %>% filter(metric %in% names(METRIC_NAME_MAPPING))

    for(name in names(METRIC_NAME_MAPPING)) {
        stats_boxplot$metric[stats_boxplot$metric == name] <- METRIC_NAME_MAPPING[[name]]
    }

    title <- smellsToAnalyze[[smell]]
    ggplot(data = stats_boxplot) +
        ggtitle(title) + 
        xlab("Threshold") + 
        ylab("Metric value") +
        geom_boxplot(mapping = aes(x = threshold, y = value, fill = threshold), show.legend = FALSE, na.rm = TRUE) +
